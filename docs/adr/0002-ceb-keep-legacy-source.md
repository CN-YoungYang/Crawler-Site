...
1. **ceb 维持旧源** `bulletin.cebpubservice.com` + 代理换 IP（ADR 0001 方案不变）；当前 `sites/ceb.js`/`crawler.js` 保持旧源抓取，并由 easy_proxies 多端口提供可切换出口；当前回归为 `npm test` 9/9 套件通过。
...
- 旧源代理链路（`HTTP_PROXY/CEB_PROXY_URL` + `easy_proxies` 旁车服务）仍是 ceb 可用性的关键依赖，故障时表现为连续 `ECONNRESET/timeout`，处置见 ADR 0001。
