// 边界：crawlPage 重试退避（紧贴 boundary403 分支的 catch 扩展）
...
function main() {
  ...
  // 第 0 次尝试 → [0, 2000)，第 1 次尝试 → [0, 4000)，第 2 次尝试 → [0, 8000)
  ...
  console.log('crawlPage 重试退避：通过');
}

try { main(); } catch (e) { console.error('失败', e.message); process.exit(1); }
