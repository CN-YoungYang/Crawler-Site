...
| **代理换 IP (Proxy Rotation)** | `ceb` 固定 IP 被 WAF 拦时的根治手段：编排层 `crawler.js#trySwitchProxy`（同站 Promise 链互斥、轮换记账、隧道重建，键统一 `rotateKey` 归一）+ provider `sites/_mihomo.js#switchNode`（可经 `siteConfig.switchProxy` 按站覆盖）——**叶子节点轮换**语义：只切真实叶子（**结构判定**：成员对象带 `all` 数组即组，连同 `DIRECT`/`REJECT` 排除，未来新组型自动命中），同轮不重复已试节点按序轮取；有叶子快照走单组端点 `/proxies/{group}` 省 I/O、轮尽零请求短路报 `proxy_pool_exhausted` 不再切，`proxy_pool_empty`（0 叶子）时 `refreshProviders()` 经 `PUT /providers/proxy/remote` 主动刷新订阅（限频 `MIHOMO_REFRESH_COOLDOWN=600s`，`proxy-providers interval 1800s` 定时轮询 + `mihomo/update-subscription.sh` 手动触发）。触发源为双 405（单页即换，**第一页双 405/网络失败会立即换 IP 重试该页一次**）与网络级连败（`NET_FAIL_SWITCH_THRESHOLD`=2）。Agent 缓存键 `<site>|<proxyUrl>` 按站隔离——换点只 destroy 本站旧 keepAlive Agent 隧道，全局 HTTP_PROXY 下兄弟站不受影响；多站共用同一出口时启动告警 `proxy_shared_exit`（PROXY 组是控制器级单例）。双 405 换点成功会重置连续 405 计数给新出口观察窗，成功页清空该站轮换记忆与叶子快照，每轮 `crawl()` 开始重置。 | `crawler.js#trySwitchProxy`, `sites/_mihomo.js`, `crawler.js#crawl` |
...
| **容器隔离 (One Container Multiple Sites)** | 一容器并发多站点（`SITES=yfbzb,demo`，`Promise.all`），`SITES`/`CRON_<SITE>`/`TOTAL_PAGES_<SITE>`/`HTTP_PORT`/`HTTP_ENABLED` 区分；`file/<site>/`、`logs/<site>/`、`state-<site>.json` 按站点隔离，`file/index.html` 总导航与 `file/<site>/` 报告由 `server.js` 托管于 `HTTP_PORT`（`EXPOSE 8080`，`healthcheck` 在 `/health`），`mihomo` sidecar 默认启用（`proxy-providers interval 1800s` + `healthcheck` on `9090/proxies` + `refreshProviders()` 主动刷新，`mihomo/update-subscription.sh` 手动触发），`TZ=Asia/Shanghai` 固定在镜像与 compose；`SITE` 单站点兼容。 | `Dockerfile`, `docker-compose.yml`, `server.js`, `sites/_mihomo.js` |

## 非目标/排除

- 全站 `axios` 静态抓取；`ceb` 因 WAF 固定 IP 被拦时经代理换 IP（`HTTP_PROXY/CEB_PROXY_URL` 经 `http-proxy-agent/https-proxy-agent` 注入，`mihomo` sidecar 默认启用将远端 `clash_fast.yaml` 转 `http://mihomo:7890`，`proxy-providers interval 1800s` 定时刷新 + 0 叶子时 `refreshProviders()` 主动刷新），未配时代理直连。
