...
| **代理换 IP (Proxy Rotation)** | `ceb` 固定 IP 被 WAF 拦时的根治手段：编排层 `crawler.js#trySwitchProxy`（同站 Promise 链互斥、轮换记账、隧道重建，键统一 `rotateKey` 归一）+ provider `sites/_mihomo.js#switchNode`（可经 `siteConfig.switchProxy` 按站覆盖）——**叶子节点轮换**语义：只切真实叶子（**结构判定**：成员对象带 `all` 数组即组，连同 `DIRECT`/`REJECT` 排除，未来新组型自动命中），同轮不重复已试节点按序轮取；有叶子快照走单组端点 `/proxies/{group}` 省 I/O、轮尽零请求短路报 `proxy_pool_exhausted` 不再切。触发源为双 405（单页即换）与网络级连败（`NET_FAIL_SWITCH_THRESHOLD`=2）。Agent 缓存键 `<site>|<proxyUrl>` 按站隔离——换点只 destroy 本站旧 keepAlive Agent 隧道，全局 HTTP_PROXY 下兄弟站不受影响；多站共用同一出口时启动告警 `proxy_shared_exit`（PROXY 组是控制器级单例）。双 405 换点成功会重置连续 405 计数给新出口观察窗，成功页清空该站轮换记忆与叶子快照，每轮 `crawl()` 开始重置。 | `crawler.js#trySwitchProxy`, `sites/_mihomo.js`, `crawler.js#crawl` |
