...
| **代理换 IP (Proxy Rotation)** | `ceb` 固定 IP 被 WAF 拦时的根治手段：`trySwitchProxy(siteConfig, reason)` 仅识别 mihomo 场景（`MIHOMO_CONTROLLER` 或默认 `mihomo:9090`），经 external-controller 切换 `PROXY` 组——**叶子节点轮换**语义：只切真实叶子（排除 `DIRECT` 与 Selector/URLTest/Fallback/LoadBalance/Relay 组类型），同轮不重复已试节点按序轮取，轮尽报 `proxy_pool_exhausted` 不再切；触发源为双 405（单页即换）与网络级连败（`NET_FAIL_SWITCH_THRESHOLD`=2）。换点成功 destroy 旧 keepAlive Agent 隧道；双 405 换点成功会重置连续 405 计数给新出口观察窗，成功页清空轮换记忆，每轮 `crawl()` 开始重置。 | `crawler.js#trySwitchProxy`, `crawler.js#crawl` |
