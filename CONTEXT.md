...
| **站点注册表 (Site Registry)** | `sites/index.js` 的 `registry` 与 `normalizeSite()`/`normalizeSites()`/`parseSitesList()`/`getSiteConfig()`/`listEnabledSites()`，集中校验与枚举站点（仅 `yfbzb`/`ceb`）；支持 `SITES` 多值与每站 `CRON_<SITE>`/`TOTAL_PAGES_<SITE>`/`SITES_CONFIG`。 | `sites/index.js` |
