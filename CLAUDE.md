...
- **Docker**: `Dockerfile` is `node:20-alpine` + `tzdata` + `ENV TZ=Asia/Shanghai` + `WORKDIR /app` + `USER node` + `EXPOSE 8080` + `ENTRYPOINT ["node","index.js"]`; `docker-compose.yml` is single service `crawler` with `SITES=yfbzb,site2` (comma-separated, concurrent), per-site `CRON_<SITE>`/`TOTAL_PAGES_<SITE>` or `SITES_CONFIG` JSON, bind mounts `./file:/app/file` `./logs:/app/logs`, `ports: "${HTTP_PORT:-8080}:${HTTP_PORT:-8080}"` + `healthcheck` (`node require('http').get(.../health)`), `HTTP_PORT`/`HTTP_ENABLED` env, and `CRON_EXPR` scheduling; `.github/workflows/docker-build.yml` pushes to `ghcr.io/<owner>/crawler` on `push main` / `tag v*` / `workflow_dispatch` with `npm test` gate, `gha` cache, `linux/amd64,linux/arm64`.
